...
def _getATTCompliance(fileName, sheetName, excel) -> list[OperatorItem]:
...
def _getATTChange(fileName, sheetName, type: ItemType, excel) -> dict:
    logging.info(
        "[_getATTChange][Begin] SheetName = " + sheetName + " Type = " + type.name
    )
    keyMap = {
...
def _addToMap(result: dict, type: ItemType, item: OperatorItem):
...
def _getATTItems(fileName, version, isFirstTime=False) -> dict:
    ...
    if isFirstTime == True:
        ...
        result = _group(fileName, version, excel, items)
    return result
